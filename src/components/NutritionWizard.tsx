...
import { generateDefaultWeeklyMeals } from "@/pages/DietPlanPage";
...
const NutritionWizard: React.FC<NutritionWizardProps> = ({ onComplete }) => {
  ...
  const steps = [
    {
      id: "calories",
      title: "Kalorie i posiłki",
      description: "Określ swoje potrzeby kaloryczne",
    },
    {
      id: "diet",
      title: "Preferencje dietetyczne",
      description: "Wybierz preferowany styl odżywiania",
    },
    {
      id: "restrictions",
      title: "Ograniczenia",
      description: "Alergie i niechciane składniki",
    },
    {
      id: "cooking",
      title: "Gotowanie",
      description: "Ile czasu możesz poświęcić na przygotowanie posiłków",
    },
  ];
  ...
  const generatePlan = async () => {
    ...
    try {
      ...
      if (formData.dailyCalories)
        updateData.dailyCalories = Number(formData.dailyCalories);
      if (formData.cookingTime) updateData.cookingTime = formData.cookingTime;

      await updateProfile(updateData);

      // Generuj plan żywieniowy
      let plan = await generateNutritionPlan({
        ...profile,
        ...updateData,
      });

      // Mapowanie i uzupełnianie posiłków na 7 dni po polsku
      ...
      // Wymuś zawsze pełne 7 dni z przykładowymi posiłkami, jeśli cokolwiek jest nie tak
      let weeklyMeals: Record<string, any[]> = {};
      const defaultWeekly = generateDefaultWeeklyMeals();
      const DAYS_PL = [
        "Poniedziałek",
        "Wtorek",
        "Środa",
      ...
      if (!plan.weeklyMeals || typeof plan.weeklyMeals !== "object") {
        ...
        for (const day of DAYS_PL) {
          if (
            !Array.isArray(plan.weeklyMeals[day]) ||
            plan.weeklyMeals[day].length === 0
          ) {
            isValid = false;
            break;
          }
      ...
      toast.error(
        error.message || "Błąd podczas generowania planu żywieniowego"
      );
    } finally {
          ...
          <div className="space-y-6">
            ...
            <div className="text-center mb-6">
              ...
              <h3 className="text-xl font-semibold">
                Określ swoje potrzeby żywieniowe
              </h3>
            </div>
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              ...
              <h3 className="text-xl font-semibold">
                Jakie styles odżywiania Cię interesują?
              </h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                },
                {
                  value: "vegan",
                  label: "🥬 Wegańska",
                  desc: "Tylko produkty roślinne",
                },
                {
                  value: "keto",
                  label: "🥑 Ketogeniczna",
                  desc: "Nisko węglowodanowa",
                },
                {
                  value: "balanced",
                  label: "⚖️ Zbilansowana",
                  desc: "Wszystkiego po trochu",
                },
                {
                  value: "high_protein",
                ...
                >
                  <Checkbox
                    checked={formData.dietPreferences.includes(
                      diet.value as any
                    )}
                    onCheckedChange={() =>
                      handleDietPreferenceChange(diet.value)
                    }
                    id={diet.value}
                    className="mt-1"
                  />
                  <div>
                    <Label htmlFor={diet.value} className="font-medium">
                      {diet.label}
                    </Label>
          ...
          <div className="space-y-8">
            ...
            <div>
              <Label htmlFor="disliked">
                Składniki, których nie lubisz (opcjonalne)
              </Label>
              <Input
                id="disliked"
          ...
          <div className="space-y-6">
            ...
            >
              {[
                {
                  value: "minimal",
                  label: "⚡ Minimal (15 min)",
                  desc: "Szybkie i proste posiłki",
                },
                {
                  value: "short",
                  label: "⏰ Krótki (30 min)",
                  desc: "Podstawowe gotowanie",
                },
                {
                  value: "medium",
                  label: "🍳 Średni (45-60 min)",
                  desc: "Lubię gotować od czasu do czasu",
                },
                {
                  value: "long",
                  label: "👨‍🍳 Długi (60+ min)",
                  desc: "Gotowanie to moja pasja",
                },
              ].map((time) => (
                <div
                  key={time.value}
                ...
                >
                  ...
                  />
                  <div>
                    <Label htmlFor={time.value} className="font-medium">
                      {time.label}
                    </Label>
